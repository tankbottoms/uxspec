...
.vp .vt:not([data-layer])[aria-pressed="true"] .badge,
...
/* Controls sit on their own plate rather than directly on the content: over a
   map or a rendered stage a glyph alone is not enough separation, and a
   translucent plate keeps the frame legible underneath. The plate is the
   cluster's; the marks inside it are bare, and hover is told by the cell
   lighting up and the glyph taking the accent, never by a border appearing. */
